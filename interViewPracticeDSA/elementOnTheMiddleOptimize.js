...
function letFindTheMiddle(arr) {
  ...
  for (let i = 1; i < arr.length; i++) {
    leftLoopBigvalue.push(
      Math.max(leftLoopBigvalue[leftLoopBigvalue.length - 1], arr[i])
    );
  }
  for (let i = arr.length - 2; i >= 0; i--) {
    rightLoopBigvalue.push(
      Math.min(rightLoopBigvalue[rightLoopBigvalue.length - 1], arr[i])
    );
  }
